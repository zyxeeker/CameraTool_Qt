...
LPTSTR WINAPI
StringFromResource(__out_ecount(STR_MAX_LENGTH)
LPTSTR pBuffer,
int iResourceID
);
...
#else
LPWSTR WINAPI
WideStringFromResource(__out_ecount(STR_MAX_LENGTH)
LPWSTR pBuffer,
int iResourceID
);
#endif
...
GetDialogSize(int iResourceID,     // Dialog box resource identifier
              DLGPROC pDlgProc,    // Pointer to dialog procedure
              LPARAM lParam,       // Any user data wanted in pDlgProc
              __out SIZE

*pResult);// Returns the size of dialog box
...
public:
    ...
    STDMETHODIMP CreateClipper(DWORD dwFlags, __deref_out LPDIRECTDRAWCLIPPER

    *lplpDDClipper,
    __inout_opt IUnknown
    *pUnkOuter);

    STDMETHODIMP CreatePalette(DWORD dwFlags, __in LPPALETTEENTRY lpColorTable, __deref_out LPDIRECTDRAWPALETTE

    *lplpDDPalette,
    __inout_opt IUnknown
    *pUnkOuter);

    STDMETHODIMP CreateSurface(__in LPDDSURFACEDESC lpDDSurfaceDesc, __deref_out LPDIRECTDRAWSURFACE

    *lplpDDSurface,
    __inout_opt IUnknown
    *pUnkOuter);

    STDMETHODIMP DuplicateSurface(__in LPDIRECTDRAWSURFACE lpDDSurface, __deref_out LPDIRECTDRAWSURFACE

    *lplpDupDDSurface);

    STDMETHODIMP
    EnumDisplayModes(DWORD dwSurfaceDescCount, __in LPDDSURFACEDESC lplpDDSurfaceDescList, __in LPVOID lpContext,
                     __in LPDDENUMMODESCALLBACK lpEnumCallback);

    STDMETHODIMP
    EnumSurfaces(DWORD dwFlags, __in LPDDSURFACEDESC lpDDSD, __in LPVOID lpContext, __in LPDDENUMSURFACESCALLBACK
                 lpEnumCallback);
    ...
    STDMETHODIMP GetGDISurface(__deref_out LPDIRECTDRAWSURFACE
    ...
    *lplpGDIDDSurface);
    ...
    STDMETHODIMP Initialize(__in GUID
...
public:
    ...
    STDMETHODIMP
    Blt(__in LPRECT lpDestRect, __in LPDIRECTDRAWSURFACE lpDDSrcSurface, __in LPRECT lpSrcRect, DWORD dwFlags,
    ...
    STDMETHODIMP GetAttachedSurface(__in LPDDSCAPS lpDDSCaps, __deref_out LPDIRECTDRAWSURFACE
    ...
    *lplpDDAttachedSurface);
    ...
    STDMETHODIMP GetClipper(__deref_out LPDIRECTDRAWCLIPPER

    *lplpDDClipper);

    STDMETHODIMP GetColorKey(DWORD dwFlags, __out LPDDCOLORKEY lpDDColorKey);

    STDMETHODIMP GetDC(__out HDC
    ...
    *lphDC);
    ...
    STDMETHODIMP GetPalette(__deref_out LPDIRECTDRAWPALETTE
