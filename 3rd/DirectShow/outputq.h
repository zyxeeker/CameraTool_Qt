...
public:
    ...
    *phr,          //  'Return code'
    ...
    BOOL
    bQueue = TRUE,      //  Send through queue (ignored if
    //  bAuto set)
    LONG
    lBatchSize = 1,     //  Batch
    BOOL
    bBatchExact = FALSE,//  Batch exactly to BatchSize
    LONG
    lListSize =         //  Likely number in the list
    DEFAULTCACHE,
            DWORD
    dwPriority =        //  Priority of thread to create
...
#define NEW_SEGMENT      ((IMediaSample *)(LONG_PTR)(-5))  // send NewSegment
    ...
    HANDLE m_hThread;
    __field_ecount_opt(m_lBatchSize)
    IMediaSample **m_ppSamples;
    __range(0, m_lBatchSize)
    LONG m_nBatched;
