...
STDMETHODIMP GetClassID(__out CLSID

*pClsID);

// --- IMediaFilter methods ---

STDMETHODIMP GetState(DWORD dwMSecs, __out FILTER_STATE

*State);

STDMETHODIMP SetSyncSource(__inout_opt
IReferenceClock *pClock
);

STDMETHODIMP GetSyncSource(__deref_out_opt IReferenceClock
...
: public CUnknown,        // Handles an IUnknown
...
REFCLSID
clsid);        // The clsid to be used to serialize this filter
...
STDMETHODIMP SetSyncSource(__in_opt
IReferenceClock *pClock
);
...
HRESULT ReconnectPin(IPin *pPin, __in_opt AM_MEDIA_TYPE const
...
virtual __out_opt LPAMOVIESETUP_FILTER
...
: public CUnknown, public IPin, public IQualityControl
{
...
HRESULT TryMediaTypes(
        IPin *pReceivePin,          // connect to this pin
        __in_opt const CMediaType

*pmt,  // proposed type from Connect
...
PIN_DIRECTION
dir);         // Either PINDIR_INPUT or PINDIR_OUTPUT
#ifdef UNICODE
CBasePin(
...
STDMETHODIMP Connect(
        IPin *pReceivePin,
        __in_opt const AM_MEDIA_TYPE

*pmt   // optional media type
);
...
STDMETHODIMP ConnectedTo(__deref_out IPin

**pPin);

STDMETHODIMP ConnectionMediaType(__out AM_MEDIA_TYPE
...
virtual HRESULT GetMediaType(int iPosition, __inout CMediaType
...
public:
    ...
    * pcFetched    // actual count passed returned here
    );
    ...
    STDMETHODIMP Clone(__deref_out IEnumPins
...
public:
    ...
    * pcFetched           // actual count passed
    ...
    STDMETHODIMP Clone(__deref_out IEnumMediaTypes
...
class AM_NOVTABLE CBaseOutputPin
...
virtual HRESULT DecideAllocator(IMemInputPin *pPin, __deref_out IMemAllocator
...
virtual HRESULT GetDeliveryBuffer(__deref_out IMediaSample
...
virtual HRESULT InitAllocator(__deref_out IMemAllocator
...
STDMETHODIMP GetAllocator(__deref_out IMemAllocator
...
// allocator
STDMETHODIMP GetAllocatorRequirements(__out ALLOCATOR_PROPERTIES
...
public:
    CAutoUsingOutputPin(__in CDynamicOutputPin
...
inline CAutoUsingOutputPin::CAutoUsingOutputPin(__in
CDynamicOutputPin *pOutputPin, __inout
HRESULT *phr
) :
m_pOutputPin(NULL)
        {
...
public:
    ...
    *phr,
    ...
    STDMETHODIMP GetPointer(__deref_out BYTE
    ...
    * pTimeEnd
    ...
    STDMETHODIMP GetMediaType(__deref_out AM_MEDIA_TYPE

    **ppMediaType);

    STDMETHODIMP SetMediaType(__in_opt AM_MEDIA_TYPE
...
static CMediaSample *&NextSample(__in
...
public:
    ...
    CMediaSample *Next(__in CMediaSample

    *pSample) const { return CBaseAllocator::NextSample(pSample); };

    int GetCount() const { return m_nOnList; };

    void Add(__inout CMediaSample
    ...
    *pSample)
    ...
    void Remove(__inout CMediaSample
...
CBaseAllocator(
        __in_opt
LPCTSTR ,
__inout_opt LPUNKNOWN, __inout
HRESULT *,
BOOL bEvent = TRUE, BOOL
...
STDMETHODIMP GetBuffer(__deref_out IMediaSample
...
STDMETHODIMP GetFreeCount(__out
LONG *plBuffersFree
);
...
STDAPI CreateMemoryAllocator(__deref_out IMemAllocator
