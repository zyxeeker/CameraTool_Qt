...
CBasePin::ReceiveConnection(
    ...
    DbgLog((LOG_TRACE, CONNECT_TRACE_LEVEL, TEXT(
    ...
    if (bRegister) {
        ...
        if (SUCCEEDED(hr)) {
            ...
            for (UINT m1 = 0; m1 < psetupdata->nPins; m1++) {
                hr = pIFM->RegisterPin(*(psetupdata->clsID), psetupdata->lpPin[m1].strName,
                                       psetupdata->lpPin[m1].bRendered, psetupdata->lpPin[m1].bOutput,
                                       psetupdata->lpPin[m1].bZero, psetupdata->lpPin[m1].bMany,
