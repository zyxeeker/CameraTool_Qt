...
protected:
    HANDLE m_hEvent;
public:
    CAMEvent(BOOL fManualReset = FALSE, __inout_opt HRESULT

    *
    phr = NULL
    );

    CAMEvent(__inout_opt HRESULT
...
public:
    ...
    CAMMsgEvent(__inout_opt HRESULT
...
GetTrueColorType(const BITMAPINFOHEADER *pbmiHeader);

STDAPI_(const GUID)

GetBitmapSubtype(const BITMAPINFOHEADER *pbmiHeader);

STDAPI_(WORD)

GetBitCount(const GUID *pSubtype);
...
GetSubtypeNameA(const GUID *pSubtype);

STDAPI_(WCHAR
*)

GetSubtypeNameW(const GUID *pSubtype);
...
GetBitmapFormatSize(const BITMAPINFOHEADER *pHeader);

STDAPI_(DWORD)

GetBitmapSize(const BITMAPINFOHEADER *pHeader);
...
STDAPI WriteBSTR(__deref_out BSTR

* pstrDest,
LPCWSTR szSrc
);

STDAPI FreeBSTR(__deref_in BSTR
...
STDAPI AMGetWideString(LPCWSTR pszString, __deref_out LPWSTR
...
IUnknown *QzAtlComPtrAssign(__deref_inout_opt IUnknown
...
MMRESULT CompatibleTimeSetEvent(UINT
uDelay,
UINT uResolution, __in
LPTIMECALLBACK lpTimeProc, DWORD_PTR
dwUser,
UINT fuEvent
);
