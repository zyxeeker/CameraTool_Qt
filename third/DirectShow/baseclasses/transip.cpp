...
HRESULT CTransInPlaceFilter::DecideBufferSize
        (IMemAllocator *pAlloc, __inout ALLOCATOR_PROPERTIES *pProperties
        ) {
...
__out_opt IMediaSample *CTransInPlaceFilter::Copy(IMediaSample *pSource) {
    ...
    hr = OutputPin()->PeekAllocator()->GetBuffer(
            &pDest, bTime ? &tStart : NULL, bTime ? &tStop : NULL, m_bSampleSkipped ? AM_GBF_PREVFRAMESKIPPED : 0
    );
...
CTransInPlaceInputPin::CTransInPlaceInputPin
        (__in_opt LPCTSTR pObjectName, __inout CTransInPlaceFilter *pFilter, __inout HRESULT *phr, __in_opt
         LPCWSTR pName
        )
        : CTransformInputPin(pObjectName,
                             pFilter,
                             phr,
                             pName), m_bReadOnly(FALSE), m_pTIPFilter(pFilter) {
    DbgLog((LOG_TRACE, 2
