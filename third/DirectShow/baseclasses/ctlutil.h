...
public:
    ...
    /* IDispatch methods */
    STDMETHODIMP GetTypeInfoCount(__out UINT
...
CMediaPosition(__in_opt
LPCTSTR,
__in_opt LPUNKNOWN
);
CMediaPosition(__in_opt
LPCTSTR,
__in_opt LPUNKNOWN, __inout
HRESULT *phr
);
...
class CPosPassThru : public IMediaSeeking, public CMediaPosition {
    ...
    HRESULT GetPeer(__deref_out IMediaPosition

    **ppMP);

    HRESULT GetPeerSeeking(__deref_out IMediaSeeking

    **ppMS);

public:

    CPosPassThru(__in_opt LPCTSTR, __in_opt LPUNKNOWN, __inout HRESULT
    ...
    *, IPin *);
    ...
    virtual HRESULT GetMediaTime(__out LONGLONG
    ...
    *pEndTime) {
    ...
    STDMETHODIMP GetCapabilities(__out DWORD

    * pCapabilities );

    STDMETHODIMP CheckCapabilities(__inout DWORD

    * pCapabilities );

    STDMETHODIMP SetTimeFormat(const GUID *pFormat);

    STDMETHODIMP GetTimeFormat(__out GUID
    ...
    *pFormat);
    ...
    STDMETHODIMP QueryPreferredFormat(__out GUID

    *pFormat);

    STDMETHODIMP ConvertTimeFormat(__out LONGLONG
    ...
    * pTargetFormat,
    ...
    STDMETHODIMP SetPositions(__inout_opt LONGLONG

    * pCurrent,
    DWORD CurrentFlags
    , __inout_opt
    LONGLONG *pStop, DWORD
    StopFlags );

    STDMETHODIMP GetPositions(__out_opt LONGLONG

    * pCurrent,
    __out_opt LONGLONG
    * pStop );

    STDMETHODIMP GetCurrentPosition(__out LONGLONG

    * pCurrent );

    STDMETHODIMP GetStopPosition(__out LONGLONG
    ...
    * pStop );
    ...
    STDMETHODIMP GetDuration(__out LONGLONG

    *pDuration);

    STDMETHODIMP GetAvailable(__out_opt LONGLONG

    *pEarliest,
    __out_opt LONGLONG
    *pLatest );

    STDMETHODIMP GetPreroll(__out LONGLONG

    *pllPreroll );

    // IMediaPosition properties
    STDMETHODIMP get_Duration(__out REFTIME

    * plength);

    STDMETHODIMP put_CurrentPosition(REFTIME llTime);

    STDMETHODIMP get_StopTime(__out REFTIME

    * pllTime);

    STDMETHODIMP put_StopTime(REFTIME llTime);

    STDMETHODIMP get_PrerollTime(__out REFTIME
    ...
    * pllTime);
    ...
    STDMETHODIMP get_CurrentPosition(__out REFTIME

    * pllTime);

    STDMETHODIMP CanSeekForward(__out LONG

    *pCanSeekForward);

    STDMETHODIMP CanSeekBackward(__out LONG
...
public:
    ...
    CRendererPosPassThru(__in_opt LPCTSTR, __in_opt LPUNKNOWN, __inout HRESULT
    ...
    *, IPin *);
    ...
    HRESULT GetMediaTime(__out LONGLONG
...
: public IBasicAudio, public CUnknown
{
CBaseDispatch m_basedisp;

public:

CBasicAudio(__in_opt
LPCTSTR,
__in_opt LPUNKNOWN
);
...
: public IBasicVideo2, public CUnknown
{
CBaseDispatch m_basedisp;

public:

CBaseBasicVideo(__in_opt
LPCTSTR,
__in_opt LPUNKNOWN
);
...
: public IVideoWindow, public CUnknown
{
CBaseDispatch m_basedisp;

public:

CBaseVideoWindow(__in_opt
LPCTSTR,
__in_opt LPUNKNOWN
);
...
class AM_NOVTABLE CSourcePosition

: public CMediaPosition
{

public:
CSourcePosition(__in_opt
LPCTSTR,
__in_opt LPUNKNOWN, __inout
HRESULT*,
__in CCritSec
...
* pCapabilities );
STDMETHODIMP CheckCapabilities(__inout
DWORD *pCapabilities
);
...
public:
    CDispParams(UINT nArgs, __in_ecount(nArgs) VARIANT *pArgs, __inout_opt HRESULT
...
public:
    ...
    * pQ,
    __in_opt LPUNKNOWN
    pUnk,               // aggregation outer unk
    __inout HRESULT
    *   phr,
    __in LPUNKNOWN
    pUnkExecutor,       // object that will execute this cmd
    REFTIME time,
            __in
    GUID *iid,
    long dispidMethod,
